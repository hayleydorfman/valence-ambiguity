 ...
 *revised 9/2022 by Hayley Dorfman for "frontload" task version
 **/

jsPsych.plugins["html-button-response-hd2"] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'html-button-response-hd2',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var html = '<div id="jspsych-html-button-response-hd2-stimulus">'+trial.stimulus+'</div>';
...
//if trial.true_feedback_L == -1, rocks on left, if trial.true_feedback_L == 1, gold on left
    ...
    if(prev_choice == 0){ //if the prev choice was the left mine
      ...
      //save valence as pos
        ...
        html += '<p class = "valence_position_left"> ? </p>';
        html += '<p class = "magnitude_position_left">'+Math.abs(trial.magnitude_L)+'</p>'; //magnitude of loss
        html += '<img src="'+trial.dirty_image+'" class ="fb_position_right">'; //dirty but really rocks
        html += '<p class = "valence_position_right"> ? </p>';
        html += '<p class = "magnitude_position_right">'+Math.abs(trial.magnitude_R)+'</p>'; //magnitude of win
        ...
        html += '<p class = "valence_position_left"> ? </p>';
        html += '<p class = "magnitude_position_left">'+Math.abs(trial.magnitude_L)+'</p>'; //magnitude of win
        html += '<img src="'+trial.dirty_image+'" class ="fb_position_right">'; //dirty but really gold
        html += '<p class = "valence_position_right"> ? </p>';
        html += '<p class = "magnitude_position_right">'+Math.abs(trial.magnitude_R)+'</p>'; //magnitude of win
        ...
        if(trial.true_feedback_R == -1 && trial.true_feedback_L == -1 && trial.latent_intervention == 0){ //if feedback should be rocks and latent should be known
          ...
          //save valence as pos
            ...
            html += '<p class = "valence_position_right"> ? </p>';
            html += '<p class = "magnitude_position_right">'+Math.abs(trial.magnitude_R)+'</p>'; //magnitude of loss
            html += '<img src="'+trial.dirty_image+'" class ="fb_position_left">'; //dirty but really rock
            html += '<p class = "valence_position_left"> ? </p>';
            html += '<p class = "magnitude_position_left">'+Math.abs(trial.magnitude_L)+'</p>'; //magnitude of win
            ...
            html += '<p class = "valence_position_right"> ? </p>';
            html += '<p class = "magnitude_position_right">'+Math.abs(trial.magnitude_R)+'</p>'; //magnitude of win
            html += '<img src="'+trial.dirty_image+'" class ="fb_position_left">'; //dirty but really gold
            html += '<p class = "valence_position_left"> ? </p>';
            html += '<p class = "magnitude_position_left">'+Math.abs(trial.magnitude_L)+'</p>'; //magnitude of win
    ...
    html += '<div id="jspsych-html-button-response-hd2-btngroup">';
    for (var i = 0; i < trial.choices.length; i++) {
      var str = buttons[i].replace(/%choice%/g, trial.choices[i]);
      html += '<div class="jspsych-html-button-response-hd2-button" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'" id="jspsych-html-button-response-hd2-button-' + i +'" data-choice="'+i+'">'+str+'</div>';
    }
    html += '</div>';
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      display_element.querySelector('#jspsych-html-button-response-hd2-button-' + i).addEventListener('click', function(e){
    ...
    function after_response(choice) {
      ...
      response.rt = rt;
//console.log('jspsych-html-button-response-hd2-stimulus' + display_element.querySelector('#jspsych-html-button-response-hd2-stimulus'))
      // after a valid response, the stimulus will have the CSS class 'responded'
      // which can be used to provide visual feedback that a response was recorded
      display_element.querySelector('#jspsych-html-button-response-hd2-stimulus').className += ' responded';

      // disable all the buttons after a response
      var btns = document.querySelectorAll('.jspsych-html-button-response-hd2-button button');
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
        display_element.querySelector('#jspsych-html-button-response-hd2-stimulus').style.visibility = 'hidden';
