function [lik,latents] = lik_ambigcounter_2LR_bias(x,data)
...
for n = 1:data.N
    ...
    if ~data.ambig(n)  % unambiguous trial -> normal 2LR update
        a_uc  = pick_alpha(rpe(uc));
        v(uc) = v(uc) + a_uc * rpe(uc);
    else               % ambiguous trial -> update like ambiguous chosen trials
