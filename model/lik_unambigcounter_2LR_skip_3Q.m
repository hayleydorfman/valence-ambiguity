function [lik,latents] = lik_unambigcounter_2LR_skip_3Q(x,data)
...
% UNAMBIGUOUS. AMBIGUOUS chosen outcomes are SKIPPED (no value update).
%
% USAGE: [lik,latents] = lik_unambigcounter_2LR_skip_3Q(x,data)
%
% INPUTS:
%   x - parameters:
...
q_poor = x(5);              % Q init for poor cond
q_rich = x(6);              % Q init for rich cond
q_neutral = x(7);              % Q init for neutral cond
...
for n = 1:data.N
    ...
    if n==1 || data.block(n) ~= data.block(n-1)
        % --- initialize Q-values by condition (numeric 1/2/3) ---
        % numeric coding assumed: 1=poor, 2=rich, 3=neutral
        v0 = 0;
        ...
        v = [v0, v0];                 % action values [left,right]
    ...
    % Unchosen (counterfactual) option: always unambiguous -> always update
